...
function GameOver(){
    document.getElementById('gover').innerHTML='<img src="https://i.ibb.co/mFGdkFQ/Game-Over-Final.png" height="735px" width="735px"></img>';  
    finalscore=level-1;
    setTimeout(() =>{
...
function HighScores(){
  ...
  max=ascores.at(0);
  sortedscores.push(ascores.at(0));
  sortednames.push(scorenames.at(0));
 for (let i=1; i<ascores.length; i++){ 
  if (ascores.at(i)==1){
    ...
    if (ascores.at(i)>max){
      max=ascores.at(i);
      sortedscores.splice(0, 0, ascores.at(i));
      sortednames.splice(0, 0, scorenames.at(i));
    }
...
function SubmitScore(){
  ...
  document.getElementById("isubmit").style.zIndex=7;
}
...
let manuever='no';
  if (newid==100){
    if (document.getElementById(100).className=='finish'){
       execute=1;
    }
  }
  if (execute!=1){
    ...
    <img src="https://i.ibb.co/RH680TV/Space-Balls.png" style={{position: 'absolute', zIndex: 1, height: 600, width: 1100, top: -130, left: 80}}></img>
      ...
      <div className="App">
        {(() => {
         ...
         {(() => { 
        submit.push(<div className="submit" id='isubmit' style={{ position: 'absolute', fontSize:30, color:'green', zIndex: -3, left: 363, top: 292}}><div style={{position: 'absolute', width: 190, fontWeight:800, top: 1, left: -6}}>Enter Name</div><input style={{position:'absolute', left: 180, top: -10}} type="text" class="user" id="user"/><br /><button className='submitbox' id='submitbox'  style={{position:'absolute', top: 65, left: 70, width: 160}} onClick={GetScore}>Submit Score</button>
  </div>)
          return submit;
      })()};
        {(() => { 
        score.push(<div className="score" id='iscore' style={{ position: 'absolute', fontSize:30, color:'green', fontWeight: 'bold', zIndex: 7, left: 30, top: 294}}>Score<br /><div class="scorenum" id="iscorenum" style={{ position: 'absolute', left: 32, fontSize:20, color:'yellow', fontWeight: 'bold'}}></div></div>)
          return score;
      })()};
       {(() => { 
        highscores.push(<div className="highscores" id='ihighscores' style={{ position: 'absolute', fontSize:30, color:'green', fontWeight: 'bold', zIndex: -3, left: 30, top: 370}}>HighScores<br /><div class="highscorenames" id="ihighscorenames" style={{ position: 'absolute', left: 3, textAlign: 'left', fontSize:20, color:'purple', fontWeight: 'bold'}}></div><div class="highscorenums" id="ihighscorenums" style={{ position: 'absolute', left: 145, fontSize:20, color:'yellow', fontWeight: 'bold'}}></div></div>)
          return highscores;
      })()};
         {(() => { 
        gameover.push(<div className="over" id='gover' style={{ position: 'absolute', zIndex: 5, left: 295.5, top: -84}}></div>)
          return gameover;
      })()};
      {(() => { 
